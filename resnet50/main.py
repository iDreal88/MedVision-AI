...
def build_model():
    ...
    cnn_model = Sequential([
        ...
        # Convert 1-channel grayscale to 3-channel for ResNet50
...
def make_gradcam_heatmap(img, model, base_model):
    """
    Computes a Grad-CAM heatmap for a given image and model.
    Handles nested models like ResNet50 inside Sequential.
    """
    try:
        last_conv_layer_name = "conv5_block3_out"
        
        # 1. Find the base_model in the sequential model
        base_layer_idx = -1
        ...
        if base_layer_idx == -1:
            print("Could not find base_model in sequential layers.")
            return None

        # 2. Create an internal model for the base part that outputs the conv layer AND the base output
        ...
        with tf.GradientTape() as tape:
            # --- Manual Forward Pass ---
            # a. Start with input and go through conversion
            x = model.get_layer('channel_conversion')(img)
            
            # b. Through internal grad model to get intermediate feature maps
            conv_outputs, base_output = internal_grad_model(x)
            
            # c. Continue through the rest of the Sequential model
            x = base_output
        ...
        if grads is None:
            print("Gradients are None. Tape connection might be broken.")
            return None
...
def main():
    ...
    print("Model saved as model_resnet50.keras")
    ...
    try:
        ...
        try:
            ...
            for i in range(5):
                ...
                # In a real scenario, you'd analyze the Grad-CAM heatmap to get findings.
                # Here we use a structured heuristic based on the prediction.
                if pred_name == 'Cancer':
