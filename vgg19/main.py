...
def make_gradcam_heatmap(img, model, last_conv_layer_name="block5_conv4"):
    """
    Computes a Grad-CAM heatmap for a given image and model.
    Handles nested VGG19 inside Sequential by manual layer traversal.
    """
    try:
        # In our Sequential model:
        # layer 0: Conv2D (adapter)
        # layer 1: VGG19 (functional model)
        # layer 2+: Top layers
        conv_adapter = model.layers[0]
        vgg_model = model.layers[1]
        top_layers = model.layers[2:]

        # Create a sub-model for VGG19 that outputs the target conv layer
...
def main():
    ...
    print("Model saved as model_vgg19.keras")
    ...
    try:
        ...
        for i in range(5):
            ...
            if heatmap is not None:
                ...
                conf_score = y_pred[i][y_pred_cls[i]] * 100
                gradcam_finding = "Significant activation" if np.max(heatmap) > 0.5 else "Moderate activation"
                gradcam_finding += f" detected in focused regions, suggesting influence from local texture/density in the {pred_name} class prediction."
                ...
                print(f"Saved Diagnosis Report for image {i} to {report_path}")
    except Exception as e:
